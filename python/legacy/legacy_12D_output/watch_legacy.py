"""
Watch a trained Unitree Go2 policy using MuJoCo's NATIVE passive viewer.

This is a lighter-weight alternative to watch_go2.py (which uses Gymnasium's "human" render).
The native viewer is GPU-accelerated and renders in its own thread, so it should hold real-time
where the wrapped renderer crawls. Shadows are disabled (the biggest GL cost in this scene).

Usage:  python watch.py [checkpoint] [--slowmo S]
          checkpoint : 1-4 (1=750k, 2=1.5M, 3=2.25M, 4=3M steps). Default 4.
          --slowmo   : playback slowdown (1.0=real-time, higher=slower). Default 1.0.
        e.g.  python watch.py 2            # watch the 1.5M-step checkpoint at real time
              python watch.py 3 --slowmo 3 # 2.25M checkpoint, third speed
...
parser.add_argument("checkpoint", nargs="?", type=int, default=4,
                    help="checkpoint index 1-4 (1=750k, 2=1.5M, 3=2.25M, 4=3M steps). Default 4.")
...
parser.add_argument("--dir", default="checkpoints",
                    help="checkpoint folder to load from -- a name relative to this script (e.g. "
                         "checkpoints-final-gait) or an absolute path. Default: checkpoints.")
...
SLOWMO = args.slowmo
steps = checkpoint * 750_000    # checkpoints are saved every 750k steps

# ---- Load the Go2 env from train_legacy.py (import by path so this works regardless of CWD). ----
_here = os.path.dirname(os.path.abspath(__file__))
# Resolve the checkpoint folder: absolute path as-is, else relative to this script.
...
# Reach the underlying MuJoCo model/data that the env mutates in place each step.
mj_model = venv.get_attr("model")[0]
mj_data = venv.get_attr("data")[0]
dt = venv.get_attr("dt")[0]      # control timestep (0.02 s = 50 Hz)

# Kill shadow casting -- the single biggest GL cost here. Comment out to restore shadows.
mj_model.light_castshadow[:] = 0
base_id = mujoco.mj_name2id(mj_model, mujoco.mjtObj.mjOBJ_BODY, "base")


def draw_ground_ray(viewer, venv):
    """Draw the env's COM->ground height ray (red rod) so you can see how far down the ground is."""
    origin = venv.get_attr("_ray_origin")[0]
    dist = float(venv.get_attr("_ray_dist")[0])
    scn = viewer.user_scn
    scn.ngeom = 0
    if dist <= 0:
        return
    ...
    p1 = p0.copy()
    p1[2] -= dist                                   # straight down to the detected ground point
    g = scn.geoms[0]
    mujoco.mjv_initGeom(g, int(mujoco.mjtGeom.mjGEOM_CAPSULE),
                        np.zeros(3), np.zeros(3), np.zeros(9),
                        np.array([1.0, 0.25, 0.2, 1.0], dtype=np.float32))
    mujoco.mjv_connector(g, int(mujoco.mjtGeom.mjGEOM_CAPSULE), 0.006, p0, p1)
    scn.ngeom = 1
...
def draw_push(viewer, venv):
    ...
    if mag < 1e-6:
        return
    scn = viewer.user_scn
    i = scn.ngeom                                   # append after whatever draw_ground_ray drew
    if i >= len(scn.geoms):
        return
    p0 = np.asarray(mj_data.xpos[base_id], dtype=np.float64).copy()
    p1 = p0 + force / mag * 0.30                     # fixed 0.3 m arrow along the push direction
    g = scn.geoms[i]
...
with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
    # Camera tracks the torso so the dog stays centered as it walks forward.
    # Side-on view (azimuth 90) shows the leg swing / gait best; tweak these to taste.
    viewer.cam.type = mujoco.mjtCamera.mjCAMERA_TRACKING
    viewer.cam.trackbodyid = base_id
    viewer.cam.distance = 2.5      # zoom: smaller = closer
    viewer.cam.azimuth = 90        # 90 = side view; 0/180 = front/back; 45 = 3/4 view
    viewer.cam.elevation = -12     # downward tilt in degrees
    next_t = time.perf_counter()
    while viewer.is_running():
        action, _ = policy.predict(obs, deterministic=True)   # best action, no exploration noise
        obs, reward, done, info = venv.step(action)           # mutates mj_data in place
        draw_ground_ray(viewer, venv)                         # red rod = COM->ground height ray
        draw_push(viewer, venv)                                # yellow arrow = active shove (if --push)
        viewer.sync()                                         # push new state to the viewer thread
        if done.any():
            if args.terrain:
                try:
                    viewer.update_hfield(0)                   # show the freshly randomized terrain
                except Exception:
                    pass
            viewer.sync()
            time.sleep(0.5)                                   # brief pause on each reset/fall
            next_t = time.perf_counter()
            continue
        # Real-time pacing: sleep only the wall time left in this frame (no drift, no over-sleep).
        next_t += step_dt
