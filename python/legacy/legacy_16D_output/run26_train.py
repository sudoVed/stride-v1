...
def _make_heightfield(nrow, ncol, rng, coarse=10, smooth_passes=6, flat_frac=0.16, feather=0.12):
    """LOW-FREQUENCY smooth heightmap in [0,1] for the terrain hfield (wide, gentle, bounded-slope
    bumps + a feathered flat spawn patch at the centre)."""
    g = rng.random((coarse, coarse))
    ...
    h *= np.clip((r - flat_frac) / feather, 0.0, 1.0)
    return h
...
# `checkpoints_ftg/` is the clean flat Tier-3 trot (Run 25); warm-start --push / --terrain from it.
...
WARM_START = ("./checkpoints_ftg/ppo_go2_3000000_steps.zip",
              "./checkpoints_ftg/ppo_go2_vecnormalize_3000000_steps.pkl")
...
DELTA_MAX = 0.10           # cycles : hard clamp on |per-leg phase offset from trot| (Run 20: 0.15->0.10; raise for terrain)
...
class Go2Env(MujocoEnv):
    ...
    def __init__(self, terrain=False, terrain_height=0.06, push=False, **kwargs):
        ...
        self._ray_origin = np.zeros(3)
        self._ray_dist = self._h_local
        ...
        self._leg_freq = np.full(N_LEGS, BASE_FREQ)   # current per-leg phase rate (exposed for viz)
    ...
    def step(self, action):
        ...
        if self._hold > 0:
            self._delta[:] = 0.0
            self._leg_freq = np.full(N_LEGS, BASE_FREQ)
        else:
            ...
            self._delta = np.clip(self._delta, -DELTA_MAX, DELTA_MAX)  # hard coordination bound
            self._leg_freq = BASE_FREQ + FREQ_RANGE * freq_act      # commanded per-leg rate (for viz)
        ...
        if holding:
            ...
            q_target = self._default_joint + _ftg_offset(self._phase) + RESIDUAL_SCALE * joint_res
